...
pOmitNA                     <- FALSE
pThresholdsUpper            <- c( 0.65, 0.75, 0.85, 0.9, 0.95 )
...
FofR1 <- function( r1, p1, epsilon1, sigma1 ){
  ...
  1 - p1 * ( ( 1 + epsilon1 * ( r1 - theta1 ) / sigma1 ) )^(-1/epsilon1) 
...
K1 <- function( r1, p1, epsilon1, sigma1 ){
  ...
  - p1/sigma1 * ( (1 + epsilon1 * (r1 - theta1)/sigma1)^(-(1+epsilon1)/epsilon1) ) * ZofR1(r1, p1, epsilon1, sigma1)^2 * exp(1 / ZofR1( r1, p1, epsilon1, sigma1))
...
K2 <- function( r2, p2, epsilon2, sigma2 ){
  ...
  - p2/sigma2 * ( (1 + epsilon2 * (r2 - theta2 )/sigma2)^(-(1+epsilon2)/epsilon2) ) * ZofR2(r2, p2, epsilon2, sigma2)^2 * exp(1 / ZofR2( r2, p2, epsilon2, sigma2))
...
# write to file
filename             <- paste(c(pPathOut,"\\pos_excedances.csv"), collapse = "")
write.csv(x = dfOut, file = filename, row.names = FALSE, na = "")

filename             <- paste(c(pPathOut,"\\pos_excedances_tstats.csv"), collapse = "")
